...
import com.hikvision.geomesa.index.index.s2.{S2Index, S2IndexValues}
import com.hikvision.geomesa.index.index.s3.{S3Index, S3IndexValues}
...
class HBaseIndexAdapter(ds: HBaseDataStore) extends IndexAdapter[HBaseDataStore] with StrictLogging {
  ...
  protected def configureScans(originalRanges: Seq[Scan],
    ...
    if (coprocessor) {
      ...
      //打印ranges
      var scanIndex = 0
      while (scanIndex < groupedScans.size()) {
        val scan = groupedScans.get(scanIndex)
        println("startRow:" + Base64.encodeBase64String(scan.getStartRow))
        try {
          val ranges = scan.getFilter.asInstanceOf[FilterList].getFilters.get(0)
            .asInstanceOf[MultiRowRangeFilter].getRowRanges

          var rangeIndex = 0
          while (rangeIndex < ranges.size()) {
            println(Base64.encodeBase64String(ranges.get(rangeIndex).getStartRow))
            println(Base64.encodeBase64String(ranges.get(rangeIndex).getStopRow))
            rangeIndex += 1
          }
        } catch {
        ...
        println("stopRow:" + Base64.encodeBase64String(scan.getStopRow))

        scanIndex += 1
      }

      // 打印ranges大小
      var scanRangeIndex = 0
      while (scanRangeIndex < groupedScans.size()) {
        val scan = groupedScans.get(scanRangeIndex)
        try {
          val ranges = scan.getFilter.asInstanceOf[FilterList].getFilters.get(0)
            .asInstanceOf[MultiRowRangeFilter].getRowRanges
